...
# 附：复习自测题（配套《学习方法论-从看懂到掌握》）

> 本附录把全书 19 章压缩成 **自测题（带答案） + 卡壳点记录表**，专门练"主动提取"。

## 使用说明（先读这一节）

1. **学完一章 → 合上主文档**，凭记忆答本章自测题。想不起来的先跳过，标成卡壳点。
2. **再对答案**：答错的、答不全的、犹豫超过 10 秒的，都是"卡壳点"，写进卡壳点记录表。
3. **间隔复习**：按记录表的 1天 / 3天 / 7天 / 30天 打勾；每次复习**先回忆再翻答案**。
4. **代码也要回忆**：自测题之后，试着合上主文档把本章 C 代码自己敲一遍（思路即可，不必逐字）。
5. 卡壳点记录表用法：每章一个表，题号已预填；答错就在该行填"卡壳点"和"正确要点"，并在对应复习日打勾。

## 第1章 UNIX基础知识（复习自测）

### 自测题
1. 文件描述符 0/1/2 分别是什么？为什么 `open` 总返回"最小未用"的描述符？
2. `fork` 为什么"调用一次返回两次"？父、子进程各拿到什么？
3. 系统调用和库函数的本质区别？为什么"可以替换库函数、不能替换系统调用"？
4. `errno` 的两条铁律是什么？
5. 程序 vs 进程的区别？
6. 按 Ctrl-C 产生什么信号？默认动作是什么？

### 答案与要点
1. 0=标准输入，1=标准输出，2=标准出错。`open` 返回最小未用描述符，所以 shell 能靠 `close(0)` 后再 `open` 实现输入重定向。
2. 内核复制进程后，父子各返回一次：父拿到子进程 PID，子拿到 0。用返回值区分父子，分支执行不同代码。
3. 系统调用是进内核的固定入口，不可替换；库函数是 C 库包装，可替换。例：`printf` 最终调 `write`，`malloc` 底层用 `sbrk`。
4. ①只有返回值指明出错（如 -1）才查 `errno`；②任何函数都不会把 `errno` 清零。
5. 程序是磁盘上的可执行文件；进程是它的执行实例，有唯一非负 PID。
6. Ctrl-C 产生 SIGINT，默认终止进程。

### 卡壳点记录表
| 题号 | 卡壳点（我错在哪） | 正确答案要点 | 日期 | 1天 | 3天 | 7天 | 30天 |
|---|---|---|---|---|---|---|---|
| 1 | | | | | | | |
| 2 | | | | | | | |
| 3 | | | | | | | |
...
### 自测题
1. ANSI C、POSIX.1、XPG3 分别解决什么可移植性问题？
2. 为什么"运行时不确定的限制值"不能用头文件常量？用什么函数现查？
3. `off_t`、`pid_t` 这类 `_t` 类型存在的意义？
4. 功能测试宏 `_POSIX_SOURCE` / `_XOPEN_SOURCE` 是干什么的？
5. `OPEN_MAX`、`PATH_MAX`、`NAME_MAX` 各代表什么？

### 答案与要点
1. ANSI C 管 C 语言本身（语法+标准库）；POSIX.1 管操作系统界面（本书主角）；XPG3 是 POSIX 的超集（多了消息设施等）。
2. 编译时根本不知道值，不能当数组长度；用 `sysconf`（系统级）和 `pathconf`/`fpathconf`（路径/文件级）运行时查。
3. 屏蔽各系统"int 还是 long"的实现差异，写代码不用关心底层类型宽度。
4. 编译时 `-D` 开启，只暴露对应标准（如 POSIX）定义的接口，避免误用非标准函数。
5. 每进程最多打开文件数 / 路径名最大长度 / 文件名最大长度。
...
### 自测题
1. "不带缓存 I/O"是什么意思？和标准 I/O 的区别？
2. 内核三张表（进程 fd 表 / 文件表 / v 节点表）的关系？`fork` 后父子进程的 fd 如何共享？
3. 为什么多进程写文件需要原子操作？`O_APPEND` 解决了什么？
4. `O_CREAT|O_EXCL` 组合的意义？
5. `dup2(fd,1)` 和 `close(1)+fcntl(fd,F_DUPFD,1)` 等价吗？
6. 什么是文件空洞？管道上 `lseek` 返回什么？

### 答案与要点
1. `open`/`read`/`write`/`lseek`/`close` 每次调用都进内核，无用户态缓存；标准 I/O 是带缓存的封装。
2. 进程 fd 表项 → 指向文件表项（含当前位移量、状态标志）→ 指向 v 节点（文件类型+操作函数）。`fork` 后父子**共享同一文件表项**（同位移量）。
3. "定位到文件尾 + 写"是两步，可能被调度打断造成交错/覆盖。`O_APPEND` 让内核把这两步合成一个原子操作。
4. 原子地"检查文件不存在并创建"，防止两个进程都以为是自己创建了文件。
5. 不完全等价：`dup2` 是原子操作；`close`+`fcntl` 两步之间可能被信号打断。且 `dup2(fd,fd)` 直接成功。
6. 位移量超过文件长度，中间读为 0、不占磁盘块；管道/FIFO 上 `lseek` 返回 -1（设 `errno=EPIPE`）。
...
### 自测题
1. 删除一个文件需要什么权限？
2. 硬链接 vs 符号链接的区别？为什么不能对目录建硬链接？
3. `open` 后立刻 `unlink` 临时文件有什么好处？
4. `st_mtime` 和 `st_ctime` 的区别？`chmod` 改的是哪个？
5. 目录的"执行位"和"读位"分别管什么？
6. SUID 位的作用与风险？`passwd` 为什么需要它？

### 答案与要点
1. 只需对文件**所在目录**有写+执行权限，对文件本身权限无要求。
2. 硬链接共享同一 i 节点（链接计数），不能跨文件系统、不能指向目录（防循环）；符号链接是独立文件，内容存目标路径，可跨文件系统。
3. 进程崩溃也不残留临时文件；内容在最后一个 fd 关闭时才真正释放。
4. mtime=内容被修改；ctime=i 节点被修改（chmod/chown 等）。`chmod` 改 ctime，不改 mtime。
5. 执行位=搜索/进入权限（能 `cd` 进去）；读位=能列出目录里的文件名。
6. 执行时进程有效 uid 变为文件属主（passwd 借此改 /etc/shadow）。风险：SUID 程序有漏洞就会被提权，写时要极谨慎。
...
### 自测题
1. 全缓存 / 行缓存 / 不带缓存的区别？各自默认出现在什么场景？
2. 为什么 `gets` 不能用？`fgets` 安全在哪？
3. 标准 I/O 为什么通常比裸 `read(1字节)` 快得多？
4. `getc` 的返回值为什么必须存 `int` 而不是 `char`？
5. `fflush` 在标准 I/O 里和终端驱动里的语义有什么不同？
6. `fdopen` 一般用在什么场景？

### 答案与要点
1. 全缓存=填满才 I/O（磁盘文件）；行缓存=遇 `\n` 才 I/O（连终端的 stdout/stdin）；不带缓存=直接写（stderr）。`setvbuf` 可改。
2. `gets` 不限制长度，输入过长会缓冲区溢出（1988 年互联网蠕虫的源头）；`fgets` 带长度上限。
3. 底层系统调用次数从数百万次降到几百次——省的是进内核的开销，不是拷贝。
4. `EOF` 是 -1；字符可能取值 0~255，存进 `char` 后 -1 与字符 0xFF 无法区分。
5. 标准 I/O 里"刷新"=把缓存写出去（写盘）；终端驱动里"刷新"=丢弃未读输入。
6. 把已有的 fd（管道、socket）包装成 `FILE*`，让标准 I/O 函数能用在它们上。
...
### 自测题
1. 为什么要有阴影口令文件？普通用户读不到加密口令有何安全意义？
2. "添加组 ID"解决了什么问题？
3. `time_t` 是什么？`localtime` 和 `gmtime` 的区别？
4. `getpwent` 这类函数返回的指针有什么坑？遍历后为什么必须 `endpwent`？
5. `TZ` 环境变量影响哪些函数？

### 答案与要点
1. 加密口令单独放 `/etc/shadow`，普通用户读不到，防止拿加密口令离线破解。只有 login/passwd 等 SUID 程序能访问。
2. 一个用户同时属于多个组，参与多个项目不必频繁 `newgrp` 切换主组。
3. `time_t` 是从 1970-1-1 起经过的秒数（UTC）；`localtime` 转成本地时区的分解时间，`gmtime` 转成 UTC 分解时间。
4. 返回的是**静态区**的 struct，下次调用会被覆盖——要保存必须先 `memcpy`；遍历后忘 `endpwent` 会泄漏文件描述符。
5. `localtime`、`ctime`、`strftime` 等一切跟时区/夏时制相关的函数。
...
### 自测题
1. `exit` 和 `_exit` 的区别？标准 I/O 缓存在什么时候被刷？
2. main 返回就等价于调用 exit 吗？为什么 main 要声明返回 `int`？
3. C 程序的内存分哪几段？bss 为什么不占磁盘空间？
4. `setjmp`/`longjmp` 有什么坑？为什么信号处理里不能用普通的 longjmp？
5. 资源限制的软限制和硬限制区别？普通用户能提高硬限制吗？

### 答案与要点
1. `_exit` 直接进内核；`exit` 先做清理：按逆序执行 atexit 处理程序 → 刷标准 I/O 缓存 → 关流，再进内核。
2. 等价。启动例程形如 `exit(main(argc,argv))`。main 若不 return 或 exit，终止状态未定义（常是栈上残留的随机值）。
3. 正文段 / 初始化数据段 / bss / 堆 / 栈。bss 里是未初始化数据，启动时由内核清零，不必存在磁盘文件里。
4. `longjmp` 后自动/寄存器变量的值不确定，要跨跳转保持值必须用 `volatile`；信号处理里要用 `sigsetjmp`/`siglongjmp`（保证信号屏蔽字行为可移植）。
5. 软限制 ≤ 硬限制，普通进程能改软限制；硬限制普通用户只能往下降（不可逆），只有 root 能提高硬限制。
...
### 自测题
1. `fork` 一次返回两次？子进程为什么返回 0？
2. 写时复制（COW）是什么？为什么 fork 后通常不立刻拷贝地址空间？
3. fork 后父子进程的文件描述符如何共享？有什么坑？
4. 僵死进程怎么产生？怎么避免？为什么 init 领养的进程不会僵死？
5. `exec` 后哪些属性保留、哪些改变？
6. 设置-用户-ID（SUID）程序为什么不能用 `system`？

### 答案与要点
1. 内核复制进程，父子各返回一次：父得子 PID，子得 0。用返回值区分父/子分支。
2. fork 后父子共享只读页，谁要写就拷贝该页——省时间省内存；数据/堆/栈因此是"延迟复制"。
3. 父子共享**同一文件表项**（同位移量），所以写同一 fd 会交织；要各自关闭不用的 fd 或加同步。
4. 子先死、父没 `wait` → 内核保留 pid+状态等成僵死（ps 显示 Z）。避免：及时 wait、或 fork 两次让 init 收养。init 对收养的子进程会 `wait`，所以不会僵死。
5. 保留：pid、uid/gid、进程组、会话、cwd、信号屏蔽、资源限制、已打开且没设 FD_CLOEXEC 的 fd。改变：正文/数据/堆/栈被新程序替换。
6. `system` 内部 `fork+exec(/bin/sh -c)`，shell 会继承 SUID 特权，成为提权后门。要用 `fork+exec` 并在 exec 前先降权。
...
### 自测题
1. 进程组、会话、控制终端三者的关系？
2. 为什么 Ctrl-C 只杀掉前台进程组？
3. 孤儿进程和孤儿进程组的区别？孤儿进程组被怎么处理？
4. `setsid` 为什么要先 `fork`？
5. 作业控制 shell 和普通 Bourne shell 在 exec 管道时进程组织有什么不同？

### 答案与要点
1. 会话包含一个或多个进程组；控制终端归会话所有（可有一个）；会话里有一个前台进程组 + 若干后台进程组，终端信号只送前台组。
2. 终端驱动把中断/退出/挂起字符产生的信号只发给当前前台进程组，后台组收不到。
3. 孤儿进程=父死了被 init 领养；孤儿进程组=组内每个成员的父都不在本会话。若组内有**停止**的进程，内核先发 SIGHUP 再发 SIGCONT（否则永远停住）。
4. 若调用者已是进程组长，`setsid` 会失败；`fork` 后子进程必不是组长，可以顺利建新会话。
5. 普通 shell 让管道里所有进程同属一个进程组；作业控制 shell 每个作业放独立进程组，前台作业组=控制终端的前台组。
...
### 自测题
1. 信号是什么？三种处理方式分别是什么？
2. 早期不可靠信号的问题？POSIX 可靠信号怎么解决？
3. `SA_RESTART` 的作用？为什么 SIGALRM 常常不重启被中断的系统调用？
4. 信号处理函数里为什么不能随便调 `printf`？什么是异步信号安全？
5. 临时阻塞信号，为什么还原时用 `SIG_SETMASK` 而不是 `SIG_UNBLOCK`？
6. `sigsuspend` 解决了什么竞态？

### 答案与要点
1. 信号是软件中断/异步事件通知。处理方式：默认动作 SIG_DFL / 忽略 SIG_IGN / 捕获（自定义处理函数）。
2. 早期问题：信号可能丢失；处理中无法可靠阻塞同类信号；处理完动作被复位成默认。POSIX 解决：不丢失、`sa_mask` 自动阻塞同类、动作持久不复位。
3. `SA_RESTART` 让被信号中断的慢系统调用自动重启。SIGALRM 常用于超时，若重启则超时机制失效，所以不重启。
4. 处理函数会打断主流程任意位置，`printf`/`malloc` 不可重入，会破坏状态。异步信号安全函数才能调用，最安全做法是只设 `volatile sig_atomic_t` 全局标志。
5. 调用者可能本来就已经阻塞该信号；用 `SIG_UNBLOCK` 会误把调用者原本的阻塞也放开。正确做法：保存旧屏蔽字，用 `SIG_SETMASK` 还原。
6. 解决"解除阻塞 + 等待信号"两步之间的竞态（可能丢信号）。`sigsuspend` 把"替换屏蔽字 + 挂起等待"做成原子操作。
...
### 自测题
1. 规范方式 vs 非规范方式？`ICANON` 控制什么？
2. EOF / INTR / KILL 三个特殊字符各起什么作用？为什么"禁止中断字符"不等于"忽略 SIGINT"？
3. `tcsetattr` 的 `TCSAFLUSH` 选项语义？
4. 非规范方式下 MIN/TIME 的四种情形，各自什么时候让 read 返回？
5. 写改终端属性的程序，最容易犯的致命错误是什么？

### 答案与要点
1. 规范方式（ICANON 开）按行装配，read 一次最多返回一行；非规范方式（ICANON 关）不装配，靠 MIN/TIME 决定返回时机。
2. EOF（Ctrl-D）=文件结束；INTR（Ctrl-C）=产生 SIGINT；KILL=清除当前行。禁止字符只是让"敲键盘"不再产生信号，信号仍可由 `kill()` 系统调用发来，所以不相等。
3. 等输出全部完成后再改属性，并丢弃未读的输入。
4. MIN>0,TIME>0=字节间计时器；MIN>0,TIME=0=凑够 MIN 才返回（可能永久阻塞）；MIN=0,TIME>0=读计时器；MIN=0,TIME=0=有数据立即返回。
5. 改之前不 `tcgetattr` 保存原值，退出（或被信号打断）前不恢复 → 终端"坏掉"（只能用 `reset` 救）。
...
### 自测题
1. 阻塞 vs 非阻塞 I/O？非阻塞 read 返回 `EAGAIN` 该怎么办？
2. `select` 和 `poll` 的区别？`select` 的 `maxfdp1` 是什么？
3. 记录锁的"建议性"vs"强制性"？`fcntl` 锁在 `fork` 后子进程继承吗？
4. `mmap` 的优缺点？为什么 `fork` 继承、`exec` 不继承？
5. `SIGBUS` 什么时候会来？
6. 为什么网络/终端上的 `read`/`write` 可能返回少于请求的字节？

### 答案与要点
1. 非阻塞不能完成时立即返回 -1 + `EAGAIN`（或 `EWOULDBLOCK`）；处理：重试或配合 `select`/`poll` 就绪后再读，别写死循环轮询。
2. `select` 用 `fd_set`（有 FD_SETSIZE 上限）+ `maxfdp1`（最大 fd 数 +1）；`poll` 用 `pollfd` 数组、无上限、不修改传入结构（用 revents 返回）。
3. 建议锁只约束合作进程；强制锁（SVR4，设组 ID 位+关组执行位）由内核在每个 read/write 检查。`fork` 后子进程**不继承**父的锁；进程终止或关闭任一相关 fd 会释放全部锁。
4. 优点：免内核↔用户的数据拷贝，大文件复制快；缺点：要管理映射生命周期、SIGBUS/SIGSEGV 风险。`fork` 共享映射（继承），`exec` 后新程序映像不继承。
5. 映射的文件被别的进程 `ftruncate` 截短后，再访问超出新长度的映射区。
6. 受缓冲、信号中断、终端行装配等影响，一次调用不一定取完全部数据——所以要用 `readn`/`writen` 循环补齐。
...
### 自测题
1. 守护进程的特征？`ps` 里怎么认出它？
2. 为什么 `fork`+`setsid`？（以及为什么建议再 fork 一次）
3. 为什么要 `chdir /` 和 `umask(0)`？
4. 守护进程怎么记日志？`syslog` 的 facility 和 level 是什么？
5. 单实例守护进程怎么实现？

### 答案与要点
1. 无控制终端（ps 的 TTY 显示 `?`）、父进程是 init、常以 root 运行、是会话/进程组首进程且通常是唯一成员。
2. `fork` 后父 exit：让 shell 认为命令完成，且子进程不是进程组长；`setsid`：建新会话、脱离控制终端。再 fork 一次：孙进程不是会话首进程，以后不可能再重新获取控制终端。
3. `chdir /` 避免占用可卸载的文件系统；`umask(0)` 让文件权限不被继承来的掩码屏蔽。
4. 用 `syslog`（openlog/syslog/closelog），经 /dev/log 送给 syslogd。facility 表示来源（LOG_DAEMON/LOG_AUTH/LOG_CRON...），level 表示严重程度（LOG_EMERG 最高到 LOG_DEBUG 最低）。
5. PID 文件 + `fcntl` 写锁：进程持有锁期间第二份副本加锁失败即退出。
...
### 自测题
1. 管道的两个限制是什么？
2. 为什么写"读端已关闭"的管道会收到 `SIGPIPE`？
3. FIFO 和普通管道的区别？
4. System V IPC 的三大缺点是什么？
5. `popen` 的原理？它和 `system` 的异同？
6. 共享存储为什么必须配同步机制？

### 答案与要点
1. 半双工（单向）；只能用于有共同祖先的进程之间。
2. 读端全部关闭后，写端再写没有意义，内核发 `SIGPIPE`；若忽略/从处理函数返回，`write` 返回 -1 且 errno=EPIPE。
3. FIFO 有文件系统名字（`mkfifo` 建），不相关的进程也能用；普通管道只在 fork 时继承。
4. 无访问计数（删了还在系统里残留，要 ipcrm）；无文件系统名字（只能用 ipcs/ipcrm 命令管理）；无文件描述符（`select`/`poll` 用不了）。
5. 都是 `fork+exec+waitpid` 的封装；`popen` 多接一根管道，type="r" 连命令的 stdout、"w" 连命令的 stdin。
6. 共享存储本身无内核互斥，多进程同时写会数据竞争；必须用信号量或记录锁做同步。
...
### 自测题
1. 为什么要在进程间传 fd，而不是传文件名？
2. 发送方关闭 fd 后，接收方为什么还能继续用？
3. 流管道 vs 普通半双工管道？SVR4 下 `pipe` 为什么天然是全双工？
4. 精灵 open 服务器相比"每客户 fork 一个进程"的优势？
5. 服务器 `select` 循环里怎么区分"新客户"和"老客户的新请求"？

### 答案与要点
1. 免搬运文件内容、可以做权限检查/权限委托（如 SUID 服务器代开文件）、两进程共享同一文件表项（同位移量）。
2. 传的是"指向同一文件表项的指针"；关闭的是发送方自己的描述符，文件表项仍被接收方引用（引用计数）。
3. 流管道全双工，一根顶两根半双工；SVR4 的 `pipe` 本身就是流管道，BSD 用 `socketpair` 实现。
4. 省掉每客户 fork/exec 的开销、是单一连接点、用 select/poll 多路并发处理所有客户。
5. `listenfd`（监听描述符）就绪 = 新客户来连；某个已连接客户 fd 就绪 = 发来新请求，read 返回 0 表示客户终止（清理）。
...
### 自测题
1. 为什么数据库用两个文件（索引 + 数据）而不是放一起？
2. 散列表 + 链表解决什么问题？
3. 集中式 vs 非集中式并发控制？本章采用哪种？
4. 粗锁 vs 细锁？细锁一定比粗锁快吗？
5. 为什么这里用 `read`/`write` 而不用标准 I/O？

### 答案与要点
1. 索引文件存散列表 + 空闲链表 + 索引记录；数据文件存记录。分开后遍历/查找索引不读整条数据，`cat` 索引也便于排障。
2. 关键字经散列函数落桶，同一桶多条记录用链表串起来，解决散列冲突。
3. 集中式=一个管理进程经 IPC 统一存取（可控但慢）；非集中式=各进程自己加锁直接读写（免 IPC 拷贝），本章采用，用记录锁实现。
4. 粗锁=锁索引文件 0 字节（锁整库），并发度最低；细锁=锁散列链+空闲链+追加区，并发度高。但进程数超过 7 后细锁收益很小，作者都怀疑值不值。
5. 标准 I/O 有缓存，别的进程改了数据，本进程可能还在用旧缓存；直接 `read`/`readv`/`write`/`writev` 保证看到最新数据。
...
## 第17章 与 PostScript 打印机通信（复习自测）

### 自测题
1. 为什么和打印机通信必须"非阻塞 + select"？
2. `select` 被信号打断（返回 EINTR）该怎么办？
3. 状态消息 `%%[ key:val ]%%` 怎么解析才稳？
4. 守护进程为什么用 `syslog` 而不用 stderr？

### 答案与要点
1. 打印机会一边收你发的 PostScript 一边随时回错误消息（全双工）；若写阻塞，就收不到错误 → 死锁。所以要非阻塞写 + select 双向多路。
2. 检查返回值是否为 -1 且 errno= EINTR，是就重新调用 select（不能依赖 SA_RESTART，它对 select 的行为因系统而异）。
3. 用字符流状态机（如 OUT/OPEN/IN 三态），识别 `%%[` 开头、`]` 结尾，把中间内容按 key:val 拆解。
4. 守护进程无控制终端，stderr 无处可去；`syslog` 经 /dev/log 可靠地交给 syslogd 落盘。
...
### 自测题
1. 为什么拨号（15~30 秒）要 `fork` 子进程来做？
2. 客户端关闭管道后，服务器怎么知道要释放锁/设备？
3. `take`（取文件）为什么必须批量读缓存？
4. 单进程 vs 双进程的终端程序，本书为什么选单进程 + `poll`？
5. `TELL_WAIT` 解决了什么竞态？

### 答案与要点
1. 拨号耗时长，若在父进程里同步做，其他客户全部被卡死；`fork` 子进程拨号，父进程继续用 select 服务其他客户。
2. 服务器读客户端连接返回 EOF（或出错），就认为客户已断开，释放锁并收回设备。
3. 逐字节读系统调用开销巨大；批量读（每次 57~58 字节）把传送时间从 16 秒降到 5 秒。
4. 双进程模型在"终止/传文件"时要进程间通知，很麻烦；单进程用 `poll` 在终端与设备间双向转发更简单可靠。
5. 避免"查到未被锁定的设备"和"fork 后才加锁"之间的竞态（先加锁再 fork）。
...
1. 伪终端（pty）解决什么问题？
2. pty 主设备与从设备是什么关系？
3. 为什么网络登录（telnetd/rlogind）需要 pty？
4. pty 怎么解决第14章的"协同进程全缓存死锁"？
5. 子进程为什么必须 `setsid` 之后再打开从设备？
6. `TIOCPKT` 和 `SIGWINCH` 分别是干什么的？

### 答案与要点
1. 让"没有真终端的地方"获得完整终端语义（行规程、回显、行编辑、控制终端）——网络登录、script、expect、驱动协同进程都靠它。
2. 从设备对程序来说就是终端（有行规程、可当控制终端）；主设备由另一进程掌控。写主设备 = 从设备收到输入，反之亦然。
3. telnetd 把 TCP 连接接到 pty 的主设备上，远端用户经 pty 从设备获得和本地终端一致的行为（回显、信号、行编辑）。
4. pty 从设备会让进程以为自己连着终端，标准 I/O 自动用行缓存，不会出现双方"全缓存互等输出"的死锁。
5. 先 `setsid` 建立新会话，之后打开从设备才能把它设为控制终端；顺序反了就不能成为控制终端。
6. TIOCPKT 打包模式：主设备感知从设备状态变化（供 rlogin）；SIGWINCH：窗口大小变化时发给前台进程组，程序据此重绘。
...
1. 从键盘按 Ctrl-C 到进程退出，中间经历了哪些章节的机制？（终端特殊字符→信号→进程终止）
2. `ls | wc -l` 这条命令，底层用了哪些系统调用，涉及哪些章节？（fork/exec/pipe/dup2/wait）
3. 一个守护进程如何做到"单实例"+"出错有日志"+"重启后不残留"？（记录锁/syslog/chdir与umask）
4. "把已打开的日志文件传给子进程写"要用什么机制？传的是什么？（SCM_RIGHTS 传 fd，共享文件表项）
5. 为什么 `pty` 能当 `script` 的底层？script 录下来的为什么没有口令？（伪终端 + 不回显）

这 5 题全部能脱口而出时，说明全书机制已串成一张网，可以进入下一本书（建议 TLPI）了。
