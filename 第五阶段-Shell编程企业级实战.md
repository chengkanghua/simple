...
```bash
Shell 管道 | 会创建子 Shell，管道后面的 while 循环运行在子进程中；
子 Shell 的变量无法回写到父 Shell，循环结束子进程销毁，父 Shell 看不到循环里修改的变量。
#错误示例
count=0
cat test.txt | while read line; do
    ((count++))
done
echo $count  # 输出依旧是0，子Shell变量带不出来
...
----------------- 代码解释 
done < eric.txt # 输入重定向方式  不会创建子 Shell
...
```bash
${arr[@]}：把数组每个元素独立拆分；加双引号后，带空格的单个元素不会被空格切割打散。
...
```bash
nohup ./test.sh > app.log 2>&1 &

&：仅后台，终端退出进程死；nohup 屏蔽挂断sighup信号，可持久运行
...
#!/bin/bash
# ==============================================================
# 生产级系统综合巡检脚本 - 串联Shell全部核心知识点
# 覆盖：变量/条件判断/循环/函数/数组/三剑客/重定向/信号/参数处理/错误处理
# ==============================================================

# 【知识点1：脚本安全选项 生产规范】
# -e: 任意命令执行失败立即退出脚本
# -u: 使用未定义变量直接报错
# -o pipefail: 管道中任意命令失败，整体管道返回失败状态码
set -euo pipefail

# ==================================================================
# 【知识点2：变量与常量】
# 全局配置变量、只读常量、字符串拼接、索引数组、关联数组
# ==================================================================
readonly SCRIPT_NAME=$(basename "$0")  # readonly 定义只读常量
LOG_DIR="/var/log/shell_monitor"
LOG_FILE="${LOG_DIR}/monitor.log"      # 字符串拼接变量
...
# 【知识点4：关联数组】存储服务名映射（bash4.0+）
declare -A SERVICE_MAP
SERVICE_MAP["ssh"]="sshd"
SERVICE_MAP["web"]="nginx"

# ==================================================================
# 【知识点5：函数定义】
# 局部变量、传参、return返回值、日志封装、通用工具函数
# ==================================================================
# 通用日志打印函数
# 入参：$1=日志级别  $2=日志内容
log() {
    local level="$1"    # local 声明函数内局部变量，不污染全局
    local content="$2"
    local time_str=$(date "+%Y-%m-%d %H:%M:%S")
    # 同时输出到控制台和日志文件
    echo "[${time_str}] [${level}] ${content}"
    echo "[${time_str}] [${level}] ${content}" >> "$LOG_FILE"
}

# ==================================================================
# 【知识点6：条件判断 + 三剑客awk/sed + 浮点运算】
# CPU负载检查
# ==================================================================
check_cpu_load() {
    log "INFO" "开始检查CPU负载..."
    # $(NF-1) 取倒数第2个字段 = 5分钟负载；sed去掉末尾逗号
    local load_5min=$(uptime | awk '{print $(NF-1)}' | sed 's/,//g')

    # bc -l 做浮点比较，1=成立 0=不成立；(( )) 整数判断结果
    if (( $(echo "$load_5min > $CPU_LOAD_THRESHOLD" | bc -l) )); then
        log "ERROR" "CPU负载异常，5分钟负载: ${load_5min}，阈值: ${CPU_LOAD_THRESHOLD}"
        return 1  # return 仅退出函数，返回状态码 1=失败
    fi

    log "INFO" "CPU负载正常，5分钟负载: ${load_5min}"
    return 0
}
...
check_memory() {
    log "INFO" "开始检查内存使用率..."
    # awk 计算使用率，保留整数
    local usage=$(free -m | awk '/Mem:/{printf "%.0f", $3/$2*100}')

    # [ ] 整数比较 -gt 大于
    if [ "$usage" -gt "$MEM_USAGE_THRESHOLD" ]; then
        log "ERROR" "内存使用率过高: ${usage}%，阈值: ${MEM_USAGE_THRESHOLD}%"
        return 1
    fi

    log "INFO" "内存使用率正常: ${usage}%"
    return 0
}

# ==================================================================
# 【知识点8：while read + 进程替换 + 字符串截断】
# 磁盘分区使用率检查
# < <() 进程替换，避免管道创建子shell导致循环内变量失效
# ==================================================================
check_disk() {
    log "INFO" "开始检查磁盘分区使用率..."
    local err_count=0

    # grep过滤掉虚拟文件系统；while read逐行读取分区和使用率
    while read -r filesystem usage_str; do
        # ${var%\%} 参数扩展，删除末尾%字符，提取纯数字
        local usage_num=${usage_str%\%}

        if [ "$usage_num" -gt "$DISK_USAGE_THRESHOLD" ]; then
            log "ERROR" "分区 ${filesystem} 使用率过高: ${usage_str}"
            ((err_count++))
        fi
    done < <(df -h | grep -vE 'tmpfs|loop|Filesystem' | awk '{print $1, $5}')

    [ "$err_count" -gt 0 ] && return 1
    log "INFO" "所有磁盘分区使用率正常"
    return 0
}

# ==================================================================
# 【知识点9：for循环遍历数组 + grep静默匹配】
# 端口存活检查
# ==================================================================
check_port_listen() {
    log "INFO" "开始检查端口监听状态..."
    ...
    # 遍历索引数组，"${arr[@]}" 保留每个元素完整性
    for port in "${CHECK_PORTS[@]}"; do
        # grep -q 静默模式，只返回状态码不输出内容
        if ss -tulnp | grep -q ":${port} "; then
            log "INFO" "端口 ${port} 正常监听"
        else
            log "ERROR" "端口 ${port} 未处于监听状态"
            ((err_count++))
        fi
    done
...
# ==================================================================
# 【知识点10：find + while read + 文件操作】
# 过期日志清理
# ==================================================================
clean_expired_log() {
    local keep_days=7
    log "INFO" "开始清理 ${keep_days} 天前的过期日志..."

    # find 按修改时间查找文件；while read 逐行处理
    find "$LOG_DIR" -type f -name "*.log.*.gz" -mtime +"$keep_days" | while read -r old_log; do
        rm -f "$old_log"
        log "INFO" "已删除过期日志: ${old_log}"
    done
...
# ==================================================================
cleanup_on_exit() {
    log "INFO" "脚本执行结束，释放资源"
}
trap cleanup_on_exit EXIT SIGINT  # 捕获正常退出、Ctrl+C中断
...
main_check() {
    ...
    local error_total=0
    # 函数返回值非0则错误数+1
    check_cpu_load    || ((error_total++))
    check_memory      || ((error_total++))
    check_disk        || ((error_total++))
    check_port_listen || ((error_total++))

    if [ "$error_total" -gt 0 ]; then
        log "ERROR" "巡检结束，共发现 ${error_total} 项异常"
        exit 1
    else
        log "INFO" "巡检结束，所有检查项全部正常"
        exit 0
    fi
}
...
| 分类   | 对应知识点                                             |
| ---- | ------------------------------------------------- |
| 基础语法 | 变量定义、只读常量、局部变量、字符串拼接、参数扩展截断                       |
| 条件判断 | `[ ]` 整数 / 字符串比较、`(( ))` 算术判断、`if/else`、浮点比较、逻辑与或 |
| 循环结构 | `for` 遍历数组、`while read` 逐行处理、进程替换规避子 Shell 问题     |
| 函数   | 定义、传参、`return` 状态码、局部变量作用域、函数调用                   |
| 数组   | 索引数组定义 / 遍历、关联数组`declare -A`、键值读写                 |
| 三剑客  | `grep -vE` 反向过滤、`awk` 字段提取 / 计算、`sed` 字符替换        |
| 进阶特性 | `trap` 信号捕获、`set` 安全选项、重定向写日志、命令替换                |
| 生产规范 | 错误码统计、日志分级、参数校验、帮助文档、资源清理                         |
